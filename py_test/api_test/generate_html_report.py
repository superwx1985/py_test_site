...
def create_step(step_list, full_case_id):
    for step in step_list:
        ...
        if status == 'p':
            ...
            step_state = 'SUCCESS'
        with tr(id='step_%s' % full_id, name='step_%s' % full_id, cid=case_id, lid=level_id, sid=step_id,
                type='tr_step', state=status):
            td(a(full_id, href='javascript:;',
               ...
               type='id_column')
            td(span(step.name, cls='pre_span'), br(), span('[%s]' % action, cls='pre_span'),
               id='step_step_column_%s' % full_id, style='border-bottom: 2px solid #AAAAAA; overflow: auto;')
            ...
            if 'response_start_time' in step.data_dict and 'response_end_time' in step.data_dict:
                response_time = round(
                    (step.data_dict['response_end_time'] - step.data_dict['response_start_time']).total_seconds(), 3)
                td(response_time, id='response_time_column_%s' % full_id,
                   style='border-bottom:2px solid #AAAAAA; text-align:center')
            else:
                td(id='response_time_column_%s' % full_id, style='border-bottom:2px solid #AAAAAA; text-align:center')
        ...
        with tr(id='step_detail_3_%s' % full_id, cid=case_id, lid=level_id, sid=step_id, type='tr_detail',
            ...
            if action == 'sub case':
                ...
                td(a('查看子用例详情', target='_blank', href='sub_case/%s.html' % sub_report_file_name,
                     onclick="this.style.color='#800080';"), style='border-bottom: 2px solid #AAAAAA;', colspan='9')
            else:
                td(div(step.message, cls='pre_div'), style='border-bottom:2px solid #AAAAAA; height:120px', colspan='9')
        # if action == 'sub case' and status != 's' and type(step.message.step_list) == list:
        #     create_step(step.message.step_list, step.message.id)
...
function toggleStepDetail(cid, full_id) {
    var id_column = document.getElementById('step_id_column_'+full_id)
    var step_column = document.getElementById('step_step_column_'+full_id)
    var state_column = document.getElementById('step_state_column_'+full_id)
    var time_column = document.getElementById('step_time_column_'+full_id)
    var response_time_column = document.getElementById('response_time_column_'+full_id)
    var detail_1 = document.getElementById('step_detail_1_'+full_id)
    var detail_2 = document.getElementById('step_detail_2_'+full_id)
    var detail_3 = document.getElementById('step_detail_3_'+full_id)
    if (arrContainsObj(show_detail, full_id)) {
        arrRemoveObj(show_detail, full_id)
        hideShareSteps(cid, full_id)
        id_column.setAttribute('rowspan',1);
        step_column.style.borderBottom='2px solid #AAAAAA';
        state_column.style.borderBottom='2px solid #AAAAAA';
        time_column.style.borderBottom='2px solid #AAAAAA';
        response_time_column.style.borderBottom='2px solid #AAAAAA';
        detail_1.className = '';
        detail_2.className = '';
        detail_3.className = '';
    }
        ...
        id_column.setAttribute('rowspan',4);
        step_column.style.borderBottom='';
        state_column.style.borderBottom='';
        time_column.style.borderBottom='';
        response_time_column.style.borderBottom='';
        detail_1.className = 'show';
        detail_2.className = 'show';
        detail_3.className = 'show';
    }
}'''
    ...
    with d.body:
        ...
        with table(id='result_table'):
            with tr(id='header_row', type='tr_header', style='text-align:center;'):
                ...
                td('全部', style='white-space:nowrap; width:7%', cls='allCase')
                td('通过', style='white-space:nowrap; width:7%', cls='passCase')
                td('失败', style='white-space:nowrap; width:7%', cls='failCase')
                td('出错', style='white-space:nowrap; width:7%', cls='errorCase')
                td('跳过', style='white-space:nowrap; width:7%', cls='skipCase')
                td('其他', style='white-space:nowrap; width:7%', cls='skipCase')
                td('响应时间', style='width:70px')
            ...
            for case_result in case_result_list:
                ...
                if case_result.status == 'p':
                    ...
                    case_class = 'skipCase'
                with tr(id='case_%s' % case_id, name=case_result.name, cls='case', type='tr_case'):
                    ...
                    td(style='border-bottom: 2px solid #AAAAAA; text-align:center; font-weight:normal')
