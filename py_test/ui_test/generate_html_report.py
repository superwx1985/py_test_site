...
def create_step(step_list, full_case_id):
    for step in step_list:
        ...
        with tr(id='step_detail_1_%s' % full_id, cid=case_id, lid=level_id, sid=step_id, type='tr_detail',
                state=status):
            td(span('Locator:', cls='blodFont'), br(), span(step.data_dict['locator'], cls='pre_span'),
               style='overflow: auto;')
            td(span('By:', cls='blodFont'), br(), span(step.data_dict['by'], cls='pre_span'), style='overflow: auto;')
            td(span('Data:', cls='blodFont'), br(), span(step.data_dict['data'], cls='pre_span'),
               style='overflow: auto;', colspan='6')
            td(span('Index:', cls='blodFont'), br(), span(step.data_dict['index'], cls='pre_span'),
               style='overflow: auto;')
