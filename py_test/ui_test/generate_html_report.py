...
def create_step(step_list, full_case_id):
    for step in step_list:
        ...
        # 如果是特殊动作，则在action后追加展示具体的特殊动作名称
        if action.lower() == 'special action':
            action = '%s | %s' % (action, step.data_dict['special_action'])
        if status == 'p':
            step_class = 'passStep'
            step_state = 'PASS'
        elif status == 's':
        ...
        with tr(id='step_%s' % full_id, name='step_%s' % full_id, cid=case_id, lid=level_id, sid=step_id,
            ...
            with td(id='step_ss_column_%s' % full_id, style='border-bottom: 2px solid #AAAAAA; overflow: auto;'):
                span(step.data_dict['last_url'], cls='pre_span')
                br()
                ss_name = step.data_dict['ss_name']
                if ss_name:
                    if ss_name == '':
                        span('截图失败')
                    else:
                        a('Screenshot', href=ss_name, onclick="this.style.color='#800080';", title=ss_name,
                          target="_blank")
        ...
        with tr(id='step_detail_1_%s' % full_id, cid=case_id, lid=level_id, sid=step_id, type='tr_detail',
                state=status):
            td(span('Locator:', cls='boldFont'), br(), span(step.data_dict['locator'], cls='pre_span'),
               style='overflow: auto;')
            td(span('By:', cls='boldFont'), br(), span(step.data_dict['by'], cls='pre_span'), style='overflow: auto;')
            td(span('Data:', cls='boldFont'), br(), span(step.data_dict['data'], cls='pre_span'),
               style='overflow: auto;', colspan='6')
            td(span('Index:', cls='boldFont'), br(), span(step.data_dict['index'], cls='pre_span'),
               style='overflow: auto;')
...
function toggleStepDetail(cid, full_id) {
    ...
    var e6 = document.getElementById('step_ss_column_'+full_id)
    var e7 = document.getElementById('step_time_column_'+full_id)
    if (arrContainsObj(show_detail, full_id)) {
        ...
        e7.style.borderBottom='2px solid #AAAAAA';
    }
    else {
    ...
    with d.body:
        ...
        with table(id='result_table'):
            with tr(id='header_row', type='tr_header', style='text-align:center;'):
                ...
                td('URL及截图', style='white-space:nowrap')
                td('全部', style='white-space:nowrap; width:50px', cls='allCase')
                td('通过', style='white-space:nowrap; width:50px', cls='passCase')
                td('失败', style='white-space:nowrap; width:50px', cls='failCase')
                td('出错', style='white-space:nowrap; width:50px', cls='errorCase')
                td('跳过', style='white-space:nowrap; width:50px', cls='skipCase')
                td('其他', style='white-space:nowrap; width:50px', cls='skipCase')
