...
# 获取配置
config = vic_config.get_config()


def run(case, variables=None, result_dir=None, base_timeout=10, get_ss=False, level=0, dr=None,
        driver_info=None):
    ...
    if not os.path.exists(result_dir):
        ...
        try:
            ...
            pass
    case_result = test_result.CaseResult(id_=case.pk, name=case.name, type_='ui')
    case_result.start_time = datetime.datetime.now()
    step_start_time = case_result.start_time
    step_id = '1'
    full_id = "%s-%s" % (case.pk, step_id)
    step_data_dict = {
        'action': 'Initialize',
        'by': '',
        'locator': '',
    ...
    if level == 0:
        logger.info('【%s】\t%s' % (case.pk, case.name))
    logger.info('【%s】\tInitialize|Loading ==> %s' % (full_id, case.name))

    try:
        # 读取测试步骤数据
        steps = Step.objects.filter(case=case).order_by('casevsstep__order')
        ...
        #     driver_info = dict()
        #     driver_info['driver_type'] = vic_method.replace_special_value(config.ui_selenium_client_list, variables)
        #     driver_info['browser_type'] = vic_method.replace_special_value(config.ui_, variables)
