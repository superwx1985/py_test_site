...
def execute_step(
    ...
    try:
        ...
        re_run = True
        while (time.time() - start_time) <= timeout and re_run:
            re_run = False
            try:
                # ===== UI =====
                # 打开URL
                ...
                # 切换frame
                    ...
                    run_result = method.try_to_switch_to_frame(
                        dr=dr, by=ui_by, locator=ui_locator, index_=ui_index, timeout=timeout,
                ...
                # 验证元素可见
                    ...
                    if ui_data == '':
                        ...
                        run_result, elements = method.wait_for_element_visible_with_data(
                            dr=dr, by=ui_by, locator=ui_locator, data=ui_data, timeout=timeout,
                            base_element=ui_base_element,
                            variable_elements=variable_elements)
                    if save_as != '':
                        variables.set_variable(save_as, elements)
                ...
                # 调用子用例
                    ...
                    if other_sub_case is None:
                        ...
                        case_result_ = execute_case(case=other_sub_case, suite_result=case_result.suite_result,
                                                    case_order=None,
                                                    user=user, execute_str=execute_id, variables=variables,
                                                    step_result=step_result, parent_case_pk_list=parent_case_pk_list,
                                                    dr=dr,
