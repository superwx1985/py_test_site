...
from main.models import DataSet, Case
from main.forms import OrderByForm, PaginatorForm, DataSetForm
...
def list_(request):
    ...
    objects = DataSet.objects.filter(q).values(
        'pk', 'uuid', 'name', 'keyword', 'project__name', 'creator', 'creator__username', 'modified_date').annotate(
        real_name=Concat('creator__last_name', 'creator__first_name', output_field=CharField()))

    # 获取被调用次数
    reference_objects1 = DataSet.objects.filter(is_active=True, case__is_active=True).values('pk').annotate(
        reference_count=Count('*'))
    reference_count1 = {o['pk']: o['reference_count'] for o in reference_objects1}

    for o in objects:
        o['reference_count'] = reference_count1.get(o['pk'], 0)
    ...
    respond = render(request, 'main/data_set/list.html', locals())
...
def detail(request, pk):
    ...
    if request.method == 'POST':
        if pk == "add":
            ...
            if not (is_admin or request.user == obj.creator):
                return HttpResponseForbidden("You don't have permission to change this object")
        if form.is_valid():
...
def multiple_delete(request):
    if request.method == 'POST':
        try:
            ...
            is_admin = check_admin(request.user)
            if is_admin:
                DataSet.objects.filter(pk__in=pk_list).update(
                    is_active=False, modifier=request.user, modified_date=timezone.now())
            else:
                DataSet.objects.filter(pk__in=pk_list, creator=request.user).update(
                    is_active=False, modifier=request.user, modified_date=timezone.now())
...
# 获取data set中的行
@login_required
def data_set_data(_, pk):
    objects = DataSet.objects.get(pk=pk)
    return JsonResponse({'state': 1, 'message': 'OK', 'data': objects.data})
...
def select_json(request):
    ...
    objects = DataSet.objects.filter(is_active=True).values('pk', 'uuid', 'name', 'keyword', 'project__name')
...
def reference(request, pk):
    try:
        ...
        raise Http404('Data Set does not exist')
    objects = Case.objects.filter(is_active=True, data_set=obj).order_by('-modified_date').values(
...
def create_table(request):
    ...
    table = DataSet.objects.create(name=table_name, column_names=columns)
